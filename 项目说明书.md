...
- 产物是静态 Markdown。Web 成功后把教程挂到首页列表，用 `/t/<项目名>` 阅读。封面 SVG 只出现在目录页；章节页不注入封面。Mermaid / highlight.js 使用 `web/static/vendor/` 本地文件，不走 CDN。
...
nodes.py             六个处理节点
utils/
  call_llm.py        LLM 调用（默认 OpenRouter glm-5.3-flash；Gemini 可选）
...
流式请求不附加额外字段；只拼接 `delta.content`。思考内容可能出现在 `reasoning`，进度可以长时间是 0 字。空内容会回退到非流式请求。
...
浏览器打开 http://127.0.0.1:8000 。**只绑定 127.0.0.1**，不要 `--host 0.0.0.0`：页面没有登录。
